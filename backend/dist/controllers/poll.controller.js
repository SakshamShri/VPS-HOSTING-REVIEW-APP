...
const poll_validator_1 = require("../validators/poll.validator");
...
class PollController {
    async create(req, res) {
        const body = poll_validator_1.pollCreateSchema.parse(req.body);
        try {
            const created = await poll_service_1.pollService.create({
                title: body.title,
    ...
    async update(req, res) {
        ...
        const body = poll_validator_1.pollUpdateSchema.parse(req.body);
        try {
            const updated = await poll_service_1.pollService.update(id, {
                title: body.title,
    ...
    async list(_req, res) {
        const items = await poll_service_1.pollService.list();
        res.json(items);
    }
    async getById(req, res) {
        const id = parseId(req);
        const poll = await poll_service_1.pollService.getById(id);
        if (!poll) {
            res.status(404).json({ message: "Poll not found" });
            return;
        }
    ...
    async getForVote(req, res) {
        const id = parseId(req);
        const detail = await poll_service_1.pollService.getForVote(id);
        if (!detail) {
            res.status(404).json({ message: "Poll not found" });
            return;
        }
    ...
    async publish(req, res) {
        const id = parseId(req);
        try {
            const published = await poll_service_1.pollService.publish(id);
            res.json(published);
        }
        catch (err) {
            if (err instanceof Error) {
    ...
    async close(req, res) {
        const id = parseId(req);
        try {
            const closed = await poll_service_1.pollService.close(id);
            res.json(closed);
        }
        catch (err) {
            if (err instanceof Error) {
    ...
    async vote(req, res) {
        ...
        const bodySchema = zod_1.z.object({
            ...
            invite_token: zod_1.z.string().optional(),
        });
        ...
        try {
            await vote_service_1.voteService.castVote({
                pollId: id,
                response: payload,
                userId,
